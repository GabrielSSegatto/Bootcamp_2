...
class EventoService:
    ...
    # Função para deletar um evento por ID e suas sessões associadas
    @staticmethod
    def delete_evento(evento_id):
        ...
        # Deletar sessões associadas ao evento
        sessoes = Sessao.query.filter_by(evento_id=evento_id).all()
    ...
    # Função para atualizar um evento por ID
    @staticmethod
    def update_evento(evento_id, nome=None):
        evento = Evento.query.get(evento_id)
        if not evento:
            raise ValueError("Evento não encontrado")
...
class SessaoService:
    ...
    @staticmethod
    def create_sessao(horario_data, is_dub, sala_id, evento_id):
        if not horario_data or is_dub is None or not sala_id  or not evento_id:
            raise ValueError("Todos os campos são obrigatórios") 
        
        evento_existente = Evento.query.get(evento_id)
        if not evento_existente:
            raise ValueError("Evento não encontrado")
        
        nova_sessao = Sessao(horario_data=horario_data, is_dub=is_dub, sala_id=sala_id, evento_id=evento_id)
    ...
    @staticmethod
    def update_sessao(sessao_id, horario_data=None, is_dub=None, sala_id=None, evento_id=None):
        sessao = Sessao.query.get(sessao_id)
        if not sessao:
            raise ValueError("Sessão não encontrada")
